...
class UserRegistrationForm(forms.ModelForm):
    ...
    password = forms.CharField(widget=forms.PasswordInput(attrs={'pattern': r'(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}'}))
    mobile = forms.CharField(widget=forms.TextInput(attrs={'pattern': '[56789][0-9]{9}'}), required=True,
                             max_length=100)
    email = forms.CharField(widget=forms.TextInput(attrs={'pattern': r'[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$'}))
